...
const postcssThemeRc = (opts = {}) => {
    ...
    const fitFlag = values.every((item) => item.length === opts.themeSelector.length);
    if (!fitFlag)
        throw new Error(`params:groups The length of the array in the object must be consistent with the length of the themeSelector`);
    // ---------------------- end 临界值 以及合法判断 ------------------------------
    ...
    return {
        ...
        Declaration(decl, { Rule }) {
            ...
            for (const [theme, color] of Object.entries(mappingValues)) {
                ...
                // 使用新的value值
                newRule.append(decl.clone({ value: color, important: !!opts.isImportant }));
            }
        }
    };
};
